...
class RatatoskrWebApiTest extends ShiroTestBase {
    ...
    void testLogin() throws Exception {
        ...
        RatatoskrWebApi servlet = simulateDSComponentActivationAndWebWhiteboardConfiguration(ratatoskr , useradmin, logservice);
    ...
    void testGetCounter() throws Exception {
        ...
        when(ratatoskr.getCounter(anyString())).thenReturn(optionalCounter);
        UserManagementService useradmin = mock(UserManagementService.class);
        RatatoskrWebApi servlet = simulateDSComponentActivationAndWebWhiteboardConfiguration(ratatoskr , useradmin, logservice);
    ...
    void testIncrementCounter() throws Exception {
        ...
        RatatoskrService ratatoskr = mock(RatatoskrService.class);
        Optional<CounterBean> optionalCounter = Optional.of(CounterBean.with().counter(counterValue).build());
        when(ratatoskr.incrementCounter(anyString())).thenReturn(optionalCounter);
        UserManagementService useradmin = mock(UserManagementService.class);
        RatatoskrWebApi servlet = simulateDSComponentActivationAndWebWhiteboardConfiguration(ratatoskr , useradmin, logservice);
    ...
    void testDecrementCounter() throws Exception {
        ...
        RatatoskrService ratatoskr = mock(RatatoskrService.class);
        Optional<CounterBean> optionalCounter = Optional.of(CounterBean.with().counter(counterValue).build());
        when(ratatoskr.decrementCounter(anyString())).thenReturn(optionalCounter);
        UserManagementService useradmin = mock(UserManagementService.class);
        RatatoskrWebApi servlet = simulateDSComponentActivationAndWebWhiteboardConfiguration(ratatoskr , useradmin, logservice);
    ...
    void testAvailableLocales() throws Exception {
        // Set up REST API servlet with mocked services
        RatatoskrService ratatoskr = mock(RatatoskrService.class);
        when(ratatoskr.availableLocales()).thenReturn(Collections.singletonList(Locale.forLanguageTag("nb-NO")).stream().map(l -> LocaleBean.with().locale(l).build()).collect(Collectors.toList()));
        MockLogService logservice = new MockLogService();
    ...
    void testDisplayTexts() throws Exception {
        ...
        texts.put("date", "Dato");
        when(ratatoskr.displayTexts(NB_NO)).thenReturn(texts);
        MockLogService logservice = new MockLogService();
    ...
    void testDisplayTextsWithUnknownLocale() throws Exception {
        ...
        Map<String, String> texts = new HashMap<>();
        texts.put("date", "Dato");
        when(ratatoskr.displayTexts(EN_UK)).thenThrow(MissingResourceException.class);
        MockLogService logservice = new MockLogService();
    ...
    private MockHttpServletRequest buildRequest(String resource) {
        ...
        request.setRequestURL("http://localhost:8181/ratatoskr/api" + resource);
        request.setRequestURI("/ratatoskr/api" + resource);
    ...
    private RatatoskrWebApi simulateDSComponentActivationAndWebWhiteboardConfiguration(RatatoskrService ratatoskr, UserManagementService useradmin, LogService logservice) throws Exception {
        RatatoskrWebApi servlet = new RatatoskrWebApi();
    ...
    private ServletConfig createServletConfigWithApplicationAndPackagenameForJerseyResources() {
        ...
        when(config.getInitParameter(ServerProperties.PROVIDER_PACKAGES)).thenReturn("no.priv.bang.ratatoskr.web.api.resources");
