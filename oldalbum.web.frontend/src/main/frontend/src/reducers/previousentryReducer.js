...
const previousentryReducer = createReducer({}, {
    ...
    [BATCH_ADD_PICTURES_FROM_URL_RECEIVE]: (state, action) => createMapFromIdToArrayOfChildren(action.payload),
    [SORT_ALBUM_ENTRIES_BY_DATE_RECEIVE]: (state, action) => createMapFromIdToArrayOfChildren(action.payload),
    [DELETE_ALBUMENTRY_RECEIVE]: (state, action) => createMapFromIdToArrayOfChildren(action.payload),
